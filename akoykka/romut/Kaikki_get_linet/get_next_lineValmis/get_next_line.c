...
/*   Updated: 2021/12/17 23:55:40 by akoykka          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

int	ft_errors(char **temp, int errortype, int first_round)
{
	if (errortype == -1)
	{	
		if (!first_round)
...
int	get_next_line(const int fd, char **line)
{
	...
	while (!strchr(temp, '\n') && r_value == BUFF_SIZE)
	{
		...
		if (r_value == -1)
			return (ft_errors(&temp, -1, first_round));
		buf[r_value] = '\0';
		free_er = temp;
		temp = ft_strjoin (temp, buf);
		if (!temp)
			return (ft_errors(&temp, -2, first_round));
		if (!first_round)
			free(free_er);
		first_round = 0;
	}
