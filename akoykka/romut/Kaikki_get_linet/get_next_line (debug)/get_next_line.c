...
/*   Updated: 2021/12/17 21:07:53 by akoykka          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_cpy(char **dest_line, char **temp, size_t r_value)
{
	char	*free_er;
	size_t	remaininglen;
	...
	*temp = (char *)ft_memmove(*temp, (strchr(*temp, '\n') + 1), remaininglen);
...
int	get_next_line(const int fd, char **line)
{
	...
	while (r_value != -1)
	{
		if (strchr(temp, '\n') || r_value < BUFF_SIZE)
		{
			return(ft_cpy(line, &temp, r_value)); //"copy finished"
		}
		while (!strchr(buf, '\n') && r_value == BUFF_SIZE)
		{
			free_er = temp;
